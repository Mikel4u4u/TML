...
Реализация нахождения данного вектора выглядит следующим образом:
...
Для решения проблемы мультиколлинеарности добавим к функционалу качества Q *регуляризатор*, штрафующий большие значения нормы вектора весов ![alt text](https://camo.githubusercontent.com/d2dea3b1855e751e2baf9dbe1c7c2dab72d7d0af/68747470733a2f2f6c617465782e636f6465636f67732e636f6d2f6769662e6c617465783f2535436c656674253230253543253743253230253543616c7068612532302535437269676874253230253543253743):

![alt text](https://latex.codecogs.com/gif.latex?Q_%5Ctau%20%28%5Calpha%29%20%3D%20%5Cleft%20%5C%7C%20F%5Calpha%20-%20y%20%5Cright%20%5C%7C%5E2%20&plus;%20%5Ctau%5Cleft%20%5C%7C%20%5Calpha%20%5Cright%20%5C%7C%5E2%2C%20%5Ctau%20%3E%200)

В случае мультиколлинеарности имеется бесконечно много векторов ![alt text](https://latex.codecogs.com/gif.latex?%5Calpha), доставляющих функционалу Q значения, близкие к минимальному. Благодаря штрафному слагаемому среди них выбирается решение с минимальной нормой.

Из равенства ![alt text](https://latex.codecogs.com/gif.latex?%5Cfrac%7B%5Cpartial%20Q_%5Ctau%7D%7B%5Cpartial%20%5Calpha%7D%20%28%5Calpha%29%20%3D%200) находим:

![alt text](https://latex.codecogs.com/gif.latex?%5Calpha_%7B%5Ctau%7D%5E*%20%3D%20%28F%5ETF%20&plus;%20%5Ctau%20I_n%29%5E%7B-1%7DF%5ETy)
...
def _findAlpha(self):
    ...
    Part1 = np.linalg.inv(np.dot(np.transpose(self.X), self.X) + np.dot(self.tau, I))
    self.alpha = np.dot(np.dot(Part1, np.transpose(self.X)), self.Y)
```

Как видно, к матрице ![alt text](https://latex.codecogs.com/gif.latex?F%5ETF) добавляется диагональная матрица ![alt text](https://latex.codecogs.com/gif.latex?%5Ctau%20I_n), которую называют *гребнем*. При этом собственные значения матрицы ![alt text](https://latex.codecogs.com/gif.latex?F%5ETF) увеличиваются на ![alt text](https://latex.codecogs.com/gif.latex?%5Ctau), но собственные векторы не изменяются.

В результате матрица ![alt text](https://latex.codecogs.com/gif.latex?F%5ETF) становится хорошо обусловленной, оставаясь в то же время "похожей" на исходную.

Регуляризованное решение задачи наименьших квадратов с использованием сингулярного разложения можно выразить следующим образом:

![alt text](https://latex.codecogs.com/gif.latex?%5Calpha_%7B%5Ctau%7D%5E*%20%3D%20%28UD%5E2U%5ET%20&plus;%20%5Ctau%20I_n%29%5E%7B-1%7DUDV%5ETy%20%3D%20U%28D%5E2%20&plus;%20%5Ctau%20I_n%29%5E%7B-1%7DDV%5ETy%20%3D%20%5Csum_%7Bj%20%3D%201%7D%5E%7Bn%7D%5Cfrac%7B%5Csqrt%7B%5Clambda_j%7D%7D%7B%5Clambda_j%20&plus;%20%5Ctau%7D%20u_j%28v_%7Bj%7D%5ETy%29)

Реализация нахождения данного вектора с применением сингулярного разложения:

```python
def _findAlpha(self):
    I = np.eye(self.X.shape[1])
    Part1 = np.linalg.inv(np.dot(np.diag(self.D), np.diag(self.D)) + np.dot(self.tau, I))
    Part2 = np.dot(np.dot(np.dot(self.U, Part1), np.diag(self.D)), np.transpose(self.V))
    self.alpha = np.dot(Part2, self.Y)
```
В качестве тестовой выборки снова был взят набор данных *Boston* по 5-му признаку — расположению жилья относительно среднего количества комнат на одно жильё.

Для подбора параметра регуляризации ![alt text](https://latex.codecogs.com/gif.latex?%5Ctau) можно использовать скользящий контроль. Практическая рекомендация — брать ![alt text](https://latex.codecogs.com/gif.latex?%5Ctau%20%5Cin%20%5B0.1%3B%200.4%5D).

Ниже представлен график зависимости LOO от параметра регуляризации:

![alt text](https://github.com/SokolovaOksana/TML/blob/master/LinearRegression/Dependence%20LOO%20from%20Tau.png)

<table>
   <tr>
       <td align = center><b>min Tau</b></td>
       <td align = center><b>min LOO</b></td>
   </tr>
   
   <tr>
      <td align = center>0.1151</td>
      <td align = center>22370.85752</td>
   </tr>
 </table>

Сравнительная таблица качества работы алгоритмов:
<table>
   <tr>
       <td align = center><b>Алгоритм</b></td>
       <td align = center><b>SSE</b></td>
   ...
   <tr>
      <td align = center><b>Ridge regression without SVD</b></td>
      <td align = center>22106.254792</td>
   </tr>
   <tr>
      <td align = center><b>Ridge regression with SVD</b></td>
      <td align = center>22064.546890</td>
   </tr>
 </table>
